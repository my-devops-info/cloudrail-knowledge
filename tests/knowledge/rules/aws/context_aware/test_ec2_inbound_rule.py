...
from cloudrail.knowledge.rules.aws.context_aware.ec2_inbound_rule import Ec2InboundRule
...
class TestEc2InboundRule(unittest.TestCase):
    def setUp(self):
        self.rule = Ec2InboundRule()

    @unittest.skip("rule param not supported")
    def test_ec2_inbound_rule_using_public_connection_fail(self):
        # Arrange
    ...
    def test_ec2_inbound_rule_using_private_connection_pass(self):
        # Arrange
    ...
    def test_ec2_inbound_rule_using_fw_pass(self):
        # Arrange
        connection_detail = PublicConnectionDetail(PolicyConnectionProperty([]), ConnectionDirectionType.INBOUND)
